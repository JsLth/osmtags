...
#' @param query A search term to filter results by. For \code{key_combinations}
...
#' @param min_fraction Numeric value specifying the fraction of all objects
#' that defines whether a value is prevalent. Defaults to 1 percent.
#' @param lang Language of the description text. Defaults to english.
#' @param type OSM geometry type of the key whose distribution should be
#' plotted. Must be either \code{"nodes"} or \code{"ways"}. Defaults to
#' \code{"nodes"}.
#' @param path Path to a file where the plot should be written to.
#' @param plot Whether to plot the distribution using the R graphics device
#' using \code{\link[showimage]{show_image}}. If \code{FALSE}, simply returns
#' the path to the file. Defaults to \code{TRUE}.
#' @param ... Additional generic parameters used for pagination, filtering,
...
key_chronology <- function(key) {
  res <- request_taginfo("key", "chronology", list(key = key))$data
  res$date <- as.POSIXct(res$date)
  as_data_frame(res)
}


#' @rdname key_chronology
#' @export
key_combinations <- function(key, query = NULL, ...) {
  force(key)
  query <- as.list(environment())
  query <- c(query, additional_options(...))
  as_data_frame(request_taginfo("key", "combinations", query = query)$data)
}


#' @rdname key_chronology
#' @export
key_distribution <- function(key,
                             type = c("nodes", "ways"),
                             path = tempfile(),
                             plot = TRUE) {
  type <- match.arg(type)
  ...
  request_taginfo("key", endpoint, query = list(key = key), path = path)
...
#' @rdname key_chronology
#' @export
key_overview <- function(key) {
  request_taginfo("key", "overview", list(key = key))$data
}


#' @rdname key_chronology
#' @export
key_prevalent_values <- function(key, min_fraction = 0.01, ...) {
  force(key)
  query <- as.list(environment())
  query <- c(query, additional_options(...))
  as_data_frame(request_taginfo("key", "prevalent_values", query)$data)
}


#' @rdname key_chronology
#' @export
key_projects <- function(key, query = NULL, ...) {
  force(key)
  query <- as.list(environment())
  query <- c(query, additional_options(...))
  as_data_frame(request_taginfo("key", "projects", query)$data)
}


#' @rdname key_chronology
#' @export
key_similar <- function(key, query = NULL, ...) {
  force(key)
  query <- as.list(environment())
  query <- c(query, additional_options(...))
  as_data_frame(request_taginfo("key", "similar", query)$data)
}


#' @rdname key_chronology
#' @export
key_stats <- function(key) {
  as_data_frame(request_taginfo("key", "stats", list(key = key))$data)
}


#' @rdname key_chronology
#' @export
key_values <- function(key, lang = "en", query = NULL, ...) {
  force(key)
  query <- as.list(environment())
  query <- c(query, additional_options(...))
  as_data_frame(request_taginfo("key", "values", query)$data)
}


#' @rdname key_chronology
#' @export
key_wiki_pages <- function(key) {
  as_data_frame(request_taginfo("key", "wiki_pages", list(key = key))$data)
}
